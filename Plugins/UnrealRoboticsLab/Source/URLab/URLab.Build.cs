...
public class URLab : ModuleRules
{
	public URLab(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			...
			// MuJoCo 3.x compatibility (symbols removed from 2.x)
			PublicDefinitions.Add("mjNPOLY=0");
			PublicDefinitions.Add("mjENBL_SLEEP=0");
			PublicDefinitions.Add("mjGAIN_DCMOTOR=mjGAIN_AFFINE");
			PublicDefinitions.Add("mjBIAS_DCMOTOR=mjBIAS_AFFINE");
			PublicDefinitions.Add("mjDYN_DCMOTOR=mjDYN_NONE");
		}
		...
		AddMuj(Target);
			// CoACD: manually add include path (symlink not followed by Directory.Exists)
			PublicIncludePaths.Add(Path.Combine(ThirdPartyPath, "CoACD", "CoACD", "include"));
		AddCoACD(Target);
		AddZeroMQ(Target);
	}
