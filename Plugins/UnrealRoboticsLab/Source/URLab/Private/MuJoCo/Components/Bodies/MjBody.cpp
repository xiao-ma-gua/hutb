...
void UMjBody::Setup(USceneComponent* Parent, mjsBody* ParentBody, FMujocoSpecWrapper* Wrapper)
{
    ...
    // 睡眠策略 — mjtSleepPolicy removed in MuJoCo 3.x
...
{
    // body_awake removed in MuJoCo 3.x — default to awake=true
    if (m_BodyView.id < 0 || !m_BodyView._d) return true;  // unbound → treat as awake
    return true; // was: m_BodyView._d->body_awake[m_BodyView.id] != 0;
}
...
//     m_BodyView._d->body_awake[m_BodyView.id] = 1;  // mjS_AWAKE
    // body_awake removed in MuJoCo 3.x
    // m_BodyView._d->body_awake[m_BodyView.id] = 1;
//    // 同时唤醒运动学树，以便物理步传播唤醒
